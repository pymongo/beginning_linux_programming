...
fn set_lock() {
    ...
    for _ in 0..BYTES_LEN {
        syscall!(write(fd, (DEFAULT_BYTE as *const u8).cast(), 1));
    }
    let current_pid = syscall!(getpid());
    ...
    syscall!(fcntl(fd, libc::F_SETLK, &shared_immutable_region));
    syscall!(fcntl(fd, libc::F_SETLK, &exclusive_mutable_region));

    println!("process {} is locking the file", syscall!(getpid()));
...
unsafe fn get_lock() {
    ...
    for start_addr in 0..BYTES_LEN {
        ...
        beginning_linux_programming::syscall!(fcntl(fd, libc::F_GETLK, &mut check_wlock_region));
